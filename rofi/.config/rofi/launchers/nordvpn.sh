...
menu() {
    ...
    if [ "$line_count" -lt 5 ]; then
        printf "%s" "$stdin" | sort | rofi -dmenu -i -p "NordVPN" -no-custom -l "$((line_count + 1))" -theme "${dir}/${theme}.rasi" -mesg "$MENU_TITLE"
    else
        printf "%s" "$stdin" | sort | rofi -dmenu -i -p "NordVPN" -no-custom -theme "${dir}/${theme}.rasi" -mesg "$MENU_TITLE"
    fi
}
